 ...
 *        queue:completed  → remove entry       (button resets to orange idle)
 *        queue:failed     → remove entry       (button resets to orange idle)
 *
...
export default function useQueueStatus() {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      // Generic handler used for all four event types
      const handleEvent = (e) => {
        try {
      ...
      es.addEventListener('queue:added',      handleEvent);
      es.addEventListener('queue:processing', handleEvent);
      es.addEventListener('queue:completed',  handleEvent);
      es.addEventListener('queue:failed',     handleEvent);
